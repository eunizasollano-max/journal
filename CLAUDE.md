...
- **Text data** (`entries`, `goals`, `recap` tables) is end-to-end encrypted client-side (see `js/crypto.js`) before it's written to Supabase, and decrypted on read — the payload column is opaque JSON (`{_enc, iv, ct}`) that the server never sees in plaintext.
...
- **Daily Routines** (`routines`, `routineLog` IndexedDB stores) are local-only for now — no Supabase sync yet, by design, while the feature is being tried out.
